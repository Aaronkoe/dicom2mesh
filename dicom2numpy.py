...
def resample(dataPath=fileHandler.dicomPath + slash + "samples", new_spacing=[1,1,1]):#was:   def resample(image, scan, new_spacing=[1,1,1], dataPath=fullPath):
  ...
  try:
      ...
      spacing = map(float, ([3] + [scan[0].PixelSpacing[0], scan[0].PixelSpacing[1]]))
      spacing = np.array(list(spacing))
      print("Seems like ya don't have a slice thickness there buddy, don't worry I went ahead and inserted 3 for ya")
